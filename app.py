...
DATA_FILE = os.path.join(BASE_DIR, "toa_anonymous_var_columns.csv")
MAPPING_FILE = os.path.join(BASE_DIR, "toa_variable_mapping_LOCAL_ONLY.csv")
...
with tab_predict:
    ...
    with col1:
        st.subheader("İlk Değerler")
        wbc_ilk = st.number_input("WBC ilk", min_value=0.0, value=12.0, step=0.1)
        nlr_ilk = st.number_input("NLR ilk", min_value=0.0, value=5.0, step=0.1)
        crp_ilk = st.number_input("CRP ilk", min_value=0.0, value=100.0, step=1.0)
        apse_ilk = st.number_input("Apse boyutu ilk", min_value=0.0, value=60.0, step=1.0)

    with col2:
        st.subheader("Kontrol Değerleri")
        wbc_kontrol = st.number_input("WBC kontrol", min_value=0.0, value=10.0, step=0.1)
        nlr_kontrol = st.number_input("NLR kontrol", min_value=0.0, value=4.0, step=0.1)
        crp_kontrol = st.number_input("CRP kontrol", min_value=0.0, value=80.0, step=1.0)
        apse_kontrol = st.number_input("Apse boyutu kontrol", min_value=0.0, value=70.0, step=1.0)
